...
`docker compose up -d --build backend frontend` applies the new token;
...
an `.env` edit; see [Database passwords and recovery](./database-passwords.md).
Moving or recloning the source repository does not
